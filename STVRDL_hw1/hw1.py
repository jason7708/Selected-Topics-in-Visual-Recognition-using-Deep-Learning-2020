...
if __name__ == '__main__':
    ...
    valid_set = torchvision.datasets.ImageFolder(root='Data/valid',
                                                 transform=valid_transform)
    train_loader = torch.utils.data.DataLoader(
        train_set, batch_size=32, shuffle=True, num_workers=2)
    valid_loader = torch.utils.data.DataLoader(
        valid_set, batch_size=32, shuffle=False, num_workers=2)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(net.parameters(), lr=3e-4, weight_decay=1e-1)
    ...
    for epoch in range(25):  # loop over the dataset multiple times
        ...
        print('train error: %.4f%%' % (
            100.*(1-correct/len(train_set))))

        # Tensor board
        writer.add_scalar(
            'Train/Loss', running_loss / (i+1), epoch)
        writer.add_scalar(
            'Train/Accuracy', 100.*correct/len(train_set), epoch)
        writer.add_scalar(
            'Train/error', 100.*(1-correct/len(train_set)), epoch)
        writer.flush()
        ...
        # Tensor board
        writer.add_scalar(
            'Validation/Loss', running_loss, epoch)
        writer.add_scalar(
            'Validation/accuracy', 100.*correct/len(valid_set), epoch)
        writer.add_scalar(
